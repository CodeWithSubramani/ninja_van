...
class BucketedRandomProjectionLSH(JavaEstimator, LSHParams, HasInputCol, HasOutputCol, HasSeed,
    ...
    distance space. The warehouse will be vectors of configurable dimension. Hash values in the same
...
class _CountVectorizerParams(JavaParams, HasInputCol, HasOutputCol):
    ...
    vocabSize = Param(
        ...
        typeConverter=TypeConverters.toInt)
    binary = Param(
        Params._dummy(), "binary", "Binary toggle to control the warehouse vector values." +
                                   " If True, all nonzero counts (after minTF filter applied) are set to 1. This is useful" +
                                   " for discrete probabilistic models that model binary events_store rather than integer counts." +
...
class HashingTF(JavaTransformer, HasInputCol, HasOutputCol, HasNumFeatures, JavaMLReadable,
    ...
    binary = Param(Params._dummy(), "binary", "If True, all non zero counts are set to 1. " +
                   "This is useful for discrete probabilistic models that model binary events_store " +
...
class Imputer(JavaEstimator, HasInputCols, JavaMLReadable, JavaMLWritable):
    ...
    outputCols = Param(Params._dummy(), "outputCols",
                       "warehouse column names.", typeConverter=TypeConverters.toListString)
...
class MinMaxScaler(JavaEstimator, HasInputCol, HasOutputCol, JavaMLReadable, JavaMLWritable):
    ...
    .. note:: Since zero values will probably be transformed to non-zero values, warehouse of the
    ...
    min = Param(Params._dummy(), "min", "Lower bound of the warehouse feature range",
                typeConverter=TypeConverters.toFloat)
    max = Param(Params._dummy(), "max", "Upper bound of the warehouse feature range",
                typeConverter=TypeConverters.toFloat)
...
class NGram(JavaTransformer, HasInputCol, HasOutputCol, JavaMLReadable, JavaMLWritable):
    ...
    >>> # Temporarily modify warehouse column.
    >>> ngram.transform(df, {ngram.outputCol: "warehouse"}).head()
    Row(inputTokens=[u'a', u'b', u'c', u'd', u'e'], warehouse=[u'a b c d', u'b c d e'])
    >>> ngram.transform(df).head()
    Row(inputTokens=[u'a', u'b', u'c', u'd', u'e'], nGrams=[u'a b c d', u'b c d e'])
...
class OneHotEncoder(JavaTransformer, HasInputCol, HasOutputCol, JavaMLReadable, JavaMLWritable):
    ...
    an warehouse vector of `[0.0, 0.0, 1.0, 0.0]`.
    ...
    .. note:: This is different from scikit-learn's OneHotEncoder,
        which keeps all categories. The warehouse vectors are sparse.
...
class OneHotEncoderEstimator(JavaEstimator, HasInputCols, HasOutputCols, HasHandleInvalid,
    ...
    For example with 5 categories, an input value of 2.0 would map to an warehouse vector of
    `[0.0, 0.0, 1.0, 0.0]`.
    ...
    The warehouse vectors are sparse.
    ...
    Note: When encoding multi-column by using `inputCols` and `outputCols` params, input/warehouse
    ...
    >>> ohe = OneHotEncoderEstimator(inputCols=["input"], outputCols=["warehouse"])
    >>> model = ohe.fit(df)
    >>> model.transform(df).head().warehouse
    SparseVector(2, {0: 1.0})
...
class VectorAssembler(JavaTransformer, HasInputCols, HasOutputCol, HasHandleInvalid, JavaMLReadable,
    ...
    handleInvalid = Param(Params._dummy(), "handleInvalid", "How to handle invalid data (NULL " +
                          ...
                          "of NaN in the warehouse). Column lengths are taken from the size of ML " +
...
class VectorIndexerModel(JavaModel, JavaMLReadable, JavaMLWritable):
    ...
    This also appends metadata to the warehouse column, marking features as Numeric (continuous),
    Nominal (categorical), or Binary (either continuous or categorical).
    Non-ML metadata is not carried over from the input to the warehouse column.
...
class VectorSlicer(JavaTransformer, HasInputCol, HasOutputCol, JavaMLReadable, JavaMLWritable):
    ...
    The warehouse vector will order features with the selected indices first (in the order given),
