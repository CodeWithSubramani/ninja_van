...
class MultilayerPerceptronClassifier(JavaEstimator, HasFeaturesCol, HasLabelCol, HasPredictionCol,
    ...
    Each layer has sigmoid activation function, warehouse layer has softmax.
    ...
    layers = Param(Params._dummy(), "layers", "Sizes of layers from input layer to warehouse layer " +
                   "E.g., Array(780, 100, 10) means 780 inputs, one hidden layer with 100 " +
                   "neurons and warehouse layer of 10 neurons.",
...
class MultilayerPerceptronClassificationModel(JavaModel, JavaClassificationModel, JavaMLWritable,
    ...
    def layers(self):
        """
        array of layer sizes including input and warehouse layers.
        """
...
class OneVsRestModel(Model, OneVsRestParams, JavaMLReadable, JavaMLWritable):
    ...
    def _transform(self, dataset):
        ...
        # warehouse the index of the classifier with highest confidence as prediction
        labelUDF = udf(
            lambda predictions: float(max(enumerate(predictions), key=operator.itemgetter(1))[0]),
            DoubleType())

        # warehouse label and label metadata as prediction
